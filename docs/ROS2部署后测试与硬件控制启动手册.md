...
| 单轮真实控制 | `pi_bringup_real_single_wheel.launch.py` | 是 | arm 服务成功后可以 |
| 四轮真实控制 | `pi_bringup_real_full_vehicle.launch.py` | 是 | arm 服务成功后可以 |
...
```text
drive mode = STOP
ControlState.armed = false
```

因此，仅启动 launch 不会发送可执行的 `enabled=true`。操作者必须在 STOP、零命令、STM32 healthy 且无锁存时调用 `/mars_rover/set_armed`。

### 2.2 当前四种驱动模式

| 模式 | 编号 | 用途 | 允许在哪种真实输出策略中使用 |
|---|---:|---|---|
| `STOP` | 0 | 全局安全停止 | `single_wheel`、`full_vehicle` |
...
- `single_wheel` 允许 STOP；运动时只允许 `RAW_WHEEL_TEST` 和配置的一个轮组。
- `full_vehicle` 允许 `STOP`、`CRAB` 和 `SPIN_IN_PLACE`，运动时允许四个轮组同时启用。
- 两种策略都使用相同的四轮消息和相同的 Pi 到 STM32 串口协议。

### 2.3 当前必须先处理的三个限制

#### 限制 A：USB 虚拟串口尚未完成现场识别

旧记录中的 `/dev/serial0` 和串口控制台问题属于已放弃的 GPIO UART 方案，不再作为当前阻塞条件。当前必须确认 STM32 通过 USB 枚举为虚拟串口。

进入串口测试前必须确认：

```bash
ls -l /dev/ttyACM* 2>/dev/null
test -e /dev/mars-rover-stm32 && readlink -f /dev/mars-rover-stm32
```

通过标准：

- STM32 USB 设备能稳定枚举为 `/dev/ttyACM*`。
- udev 稳定别名 `/dev/mars-rover-stm32` 存在。
- 当前用户具有 `dialout` 权限。
- USB 重新插拔后稳定别名保持不变。
...
#### 已修复项 C：反向 CRAB 和正反 SPIN 运动学

当前实现不再直接裁剪目标角度，而是在机械角度范围内选择等效转向角，并在需要时反转轮速。

当前纯逻辑测试已经覆盖：

- 正向、侧向和反向 CRAB。
- 正反两个 SPIN_IN_PLACE 方向。
- 由 `drive_velocity*cos(angle)` 和 `drive_velocity*sin(angle)` 重构的四轮二维速度向量。

代码层问题已修复，但实车轮组安装方向仍必须通过四轮架空测试确认。
...
| T4 | USB 枚举、权限和稳定设备名 | 断开 | STM32 通信接口可连接 | T5 |
...
| T12 | 四轮架空 `STOP/CRAB/SPIN` | 四轮支路 | 运动学向量测试通过 | T13 |
...
该测试不会打开 `/dev/mars-rover-stm32`，不会向 STM32 或电机发送命令。
...
这一阶段连接 STM32 的 USB 数据接口，但不给任何电机驱动器上电。
...
lsusb
ls -l /dev/ttyACM* 2>/dev/null
test -e /dev/mars-rover-stm32 && ls -l /dev/mars-rover-stm32
readlink -f /dev/mars-rover-stm32
groups
...
- `/dev/ttyACM*` 原始设备存在。
- `/dev/mars-rover-stm32` 稳定别名存在并指向正确设备。
- 当前用户具有串口访问权限，通常属于 `dialout` 组。
- USB 日志中没有持续枚举失败或反复掉线。
...
1. 保持电机功率断开。
2. 记录 `/dev/mars-rover-stm32` 当前指向。
3. 拔下并重新插入 STM32 USB 数据线。
4. 确认稳定别名重新出现，且仍指向该 STM32。
5. 确认没有因为编号变化而需要手工把配置改成另一个 `/dev/ttyACM*`。
...
Pi 与 STM32 逻辑部分断电后连接：

| Pi | STM32 | 说明 |
|---|---|---|
| USB Host 接口 | ST-LINK USB 或 MCU 原生 USB 数据接口 | 使用支持数据传输的 USB 线 |

如果使用 ST-LINK VCP，其对应 UART 固定为 115200、8N1、无流控。如果使用 MCU 原生 USB CDC，波特率只是主机 API 参数，不是 USB 总线速率。
...
由于 ControlState 仍为 disarmed，Pi 发给 STM32 的可执行使能必须保持关闭，电机不得动作。
...
模式管理器必须直接拒绝不属于 single-wheel profile 的 CRAB 请求，当前模式保持不变；bridge 仍保留最终策略检查作为第二道防线。
...
2. 启动 real single-wheel launch，保持 STOP/disarmed。
3. 确认 ROS 2 状态正常。
4. 接通总隔离开关和单轮驱动器支路。
5. 释放物理急停，使接触器吸合。
6. 不发送运动命令。
...
按第 12 章启动 single-wheel launch，并确认：

```bash
ros2 topic echo --once /mars_rover/control_state mars_rover_msgs/msg/ControlState
```

结果必须包含 `armed: false` 和 `motion_allowed: false`。
...
保持 STOP，确认 teleop 正在持续发布零命令。然后由急停旁人员口头确认“可以 arm”，PI-B 执行：
...
1. 调用 `/mars_rover/set_armed` 的 false 请求，确保 disarmed。
2. 键盘 `k` 并发布 STOP。
3. 确认实物停止。
4. 排查并清除故障源。
5. 确认 STATUS 恢复正常。
6. 软件急停输入恢复 false 后，调用 `/mars_rover/reset_safety`。
7. 保持 STOP 和零命令，重新取得两人确认后调用 arm；随后重新请求模式并输入新命令。
...
切换前后都必须保持 disarmed。更稳妥的正式做法是为每个轮组提供经过审核的 launch 参数，而不是在带电状态临时修改两个参数。
...
在 disarmed 和 armed 两种状态下，STOP 的四轮速度都必须为零，串口执行使能保持关闭。

### 18.4 正向 CRAB 架空测试

1. 保持 STOP 和零命令，确认 STATUS 无故障。
2. 急停旁人员确认后调用 `/mars_rover/set_armed`，检查返回 `success=true`。
3. 发布 `CRAB`，等待 `transitioning=false`，并保持键盘零命令。
4. 确认四个轮子目标角接近 `0 rad`。
5. PC-A 按 `i` 不超过 1 秒，然后按 `k`。
6. 确认四个轮子都向车辆前进方向转动。
7. 立即调用 `set_armed` 的 false 请求完成 disarm。
...
只有在状态无故障、轮组名称正确且 `armed=false` 已确认后，才接通总隔离开关、释放物理急停并使接触器吸合。
...
这一流程只适用于四轮架空验收已经通过、运动学向量测试已经通过的版本。
...
4. ControlState 必须保持 `armed=false`。
...
10. 释放物理急停。
11. 再次确认车辆仍处于 STOP。
12. 由急停人员允许后，在 STOP 和零命令下调用 arm 服务。
...
5. 再次确认 `/mars_rover/control_state` 中 `armed=false`。
...
4. 若 ROS 2 仍可用，调用 disarm 服务；不能用软件操作替代物理急停。
5. 关闭总隔离开关。
6. 等待母线放电后再检查接线。
...
```bash
ros2 topic echo /mars_rover/safety_state std_msgs/msg/String
ros2 topic echo /mars_rover/control_state mars_rover_msgs/msg/ControlState
ros2 topic echo /mars_rover/stm32/status mars_rover_msgs/msg/Stm32Status
...
| `/dev/ttyACM*` 不出现 | USB 连接和 STM32 VCP/CDC | USB 线仅供电、接口错误或固件未启动 USB 串口 |
| `/dev/mars-rover-stm32` 不存在 | udev 规则 | 序列号/VID/PID 不匹配或规则未重载 |
| `serial_connected=true` 但 `online=false` | ACK/STATUS | STM32 未回包、波特率或协议不一致 |
| arm 服务失败或 arm 后仍不动 | ControlState、STATUS、模式、零命令 | 未处于 STOP、命令未归零、存在锁存、STM32 offline 或物理急停 |
| single-wheel 请求 CRAB 被拒绝 | mode manager 日志 | `single_wheel` profile 按设计只允许 STOP/RAW |
| 只有部分轮子动作 | 地址与分支状态 | RS-485 ID 冲突、支路故障、接线映射错误 |
| 整车倒车实物方向异常 | 运动学向量测试与轮组方向映射 | ROS 数学已修复，需核对实车安装方向和底层符号 |
...
- [x] 反向 CRAB 和正反 SPIN 已完成等效角度修正与纯逻辑测试。

### 27.3 Pi 到 STM32

- [ ] `/dev/mars-rover-stm32` 可用且权限正确。
- [ ] USB 重新插拔后稳定别名和桥接重连通过。
- [ ] CRC32、ACK、STATUS 通过；使用 ST-LINK VCP 时同时确认 115200 8N1。
...
部署完成后，不应直接把 `pi_bringup_real_full_vehicle.launch.py` 当作日常第一条命令。正确顺序是：先通过跨主机 DDS 和 dry-run，随后验证命令超时与软件急停，再打通 USB 虚拟串口、STM32 ACK/STATUS 和 watchdog，最后从单轮开始接通真实功率。

当前代码已经提供 dry-run、串口联调、单轮真实控制和四轮真实控制入口，但“存在 launch 文件”不等于对应硬件已经验收。特别是：

1. STM32 USB 虚拟串口必须完成枚举、udev 稳定命名和重新插拔恢复测试。
2. `/wheel_states` 仍是目标回显，不是真实反馈。
3. 反向 `CRAB` 和正反 SPIN 已通过纯逻辑向量测试，但仍需四轮架空核对实际安装方向。
