...
| 接口 | 连接 | 功能 | 备注 |
|---|---|---|---|
| USART1 PA9/PA10 | Raspberry Pi GPIO UART ↔ STM32 | `/dev/serial0`，紧凑 JSON + CRC32 | 115200 bps，8N1 |
| USART3 + MAX485 | STM32 ↔ MKS SERVO57D，ID 1-4 | 转向电机 Modbus RTU 控制 | 115200 bps，8N1 |
| USART6 + MAX485 | STM32 ↔ BLD-305S，ID 1-4 | 行走电机 Modbus RTU 控制 | 115200 bps，8N1 |
...
- 接收 Pi 通过 USART1 GPIO UART 发送的轮组目标命令。
- 校验数据帧。
- 解析 4 个转向角和 4 个驱动速度。
- 通过 USART3 + MAX485 控制 MKS SERVO57D 转向驱动器。
- 通过 USART6 + MAX485 控制 BLD-305S。
- 管理 Modbus RTU 通信。
- 执行低层安全超时。
- 在 Pi 通信中断时停止电机。
- 返回驱动器在线状态、故障状态和可用反馈。
...
全局真实硬件使能由 `stm32_bridge.hardware_enable` 参数控制；单轮选择由 `active_test_wheel` 参数和每个 setpoint 的 `enabled` 字段表达，不再使用重复的位掩码。
...
- Pi `/dev/serial0` 使用 GPIO14 TX、GPIO15 RX 和 GND。
- STM32 使用 USART1 PA9 TX、PA10 RX。
...
- 命令超时后 FL 驱动速度变 0。
- `hardware_enable=false` 时 W 帧顶层和所有轮组执行标志均为 0。
- 软件急停时电机不转。
- 转向角正负方向与 ROS 坐标定义一致。
- 驱动速度正负方向与车体前进定义一致。
...
- `bridge_mode`
- `status_timeout_sec`
- `require_enable_for_real_serial`
- `hardware_enable`
...
6. 必须有 `/cmd_vel` 超时停车。
7. 必须有 `hardware_enable` 和 `/mars_rover/emergency_stop` 逻辑。
8. 必须发布 `/joint_states`。
