...
> ROS 2 实现更新（2026-07-03）：当前代码已加入等效角度/有符号轮速、STOP 模式过渡、急停/故障锁存、条件 arm/reset 服务、结构化 ControlState、bridge 主动安全 STOP 和 JointState 单位换算。运行操作以最新启动手册和改进记录为准。
...
- STM32G474RE（工程目标器件 STM32G474RET6）用于低层实时电机控制。
...
| 接口 | 连接 | 功能 | 备注 |
|---|---|---|---|
| USB 虚拟串口 | Raspberry Pi USB Host 到 STM32 开发板 USB 数据接口 | `/dev/mars-rover-stm32`，紧凑 JSON + CRC32 | 原始设备通常为 `/dev/ttyACM0` |
| USART1 + RS-485 | STM32 到 MKS SERVO57D，ID 1-4 | 转向电机 Modbus RTU 控制 | PC4/PC5，参数按实物确认 |
| USART3 + RS-485 | STM32 到 BLD-305S，ID 1-4 | 行走电机 Modbus RTU 控制 | PB10/PB11，BLD-305S 要求 8N1 |
...
- 安装 ROS 2。
- 创建 ROS 2 workspace。
- 使用支持数据传输的 USB 线连接 STM32，确认 Pi 出现 `/dev/ttyACM*`。
- 通过 udev 创建稳定别名 `/dev/mars-rover-stm32`，运行用户属于 `dialout` 组。
- 不再启用 Pi GPIO UART，也不连接 GPIO14/15。
...
- 配置 USB 虚拟串口，用于接收 Raspberry Pi 的高层命令。使用 ST-LINK VCP 时确认其对应 UART；使用 MCU 原生 USB 时实现 CDC ACM。
- 调试日志必须使用与正式 USB 协议分离的 UART、SWO 或其他通道。
- 配置 USART1 PC4/PC5 + RS-485 收发器，用于控制 4 个 MKS SERVO57D 转向驱动器。
- 配置 USART3 PB10/PB11 + RS-485 收发器，用于控制 4 个 BLD-305S。
- 实现 Modbus RTU 主站逻辑。
- 管理转向电机驱动器 ID 1-4。
...
Raspberry Pi -> STM32 -> USART1 -> RS-485 收发器 -> MKS SERVO57D -> NEMA23 转向步进电机 -> NMRVS30 30:1 蜗轮蜗杆减速器 -> 轮组转向轴。
...
| Pi USB Host 到 STM32 开发板 USB | `/dev/mars-rover-stm32`，紧凑 JSON + CRC32 | 必须使用支持数据传输的 USB 线，不能只使用充电线 |
| STM32 USART1 到 RS-485 收发器 | 转向 RS-485 总线，ID 1-4 | A/B 线不要接反，建议双绞线 |
| STM32 USART3 到 RS-485 收发器 | BLD-305S RS-485 总线，ID 1-4 | 与转向总线分开，便于调试 |
...
- STM32G474RE 负责底层实时执行，不需要运行完整 ROS 2。
...
- Pi USB Host 连接 STM32 开发板 USB 数据接口。
- Pi 上的 `stm32_bridge_node` 打开 `/dev/mars-rover-stm32`。
...
- 通过 `/dev/mars-rover-stm32` USB 虚拟串口发送给 STM32。
- 读取 STM32 回传帧。
...
- Pi 使用稳定别名 `/dev/mars-rover-stm32`，原始设备通常为 `/dev/ttyACM0`。
- 物理连接为 USB 数据线，不再使用 Pi GPIO TX/RX。
- 使用 ST-LINK VCP 时，VCP 对应 UART 参数保持 115200、8N1；使用原生 USB CDC 时，波特率仅作为主机接口兼容参数。
...
7. USB 虚拟串口、udev 稳定别名 `/dev/mars-rover-stm32` 和重新插拔恢复是否已按部署手册验证？
