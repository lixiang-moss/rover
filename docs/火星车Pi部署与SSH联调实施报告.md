...
- 串口 getty 仍占用 UART。
- `/dev/serial0` 尚不能作为 STM32 通信口直接使用。
...
本次未处理 UART 控制台占用，也未连接 STM32。因此不能声称以下链路已通过：

```text
Pi stm32_bridge -> /dev/serial0 -> STM32
```
