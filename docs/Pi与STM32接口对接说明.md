...
| 当前 Pi 协议内部入口 | LPUART1 | PC0 RX / PC1 TX | 已通过 ST-LINK V3 VCP 实测通信；固件使用中断环形缓冲接收 |
| SERVO57D 总线 | USART1 | PC4 TX / PC5 RX | 当前代码为 115200、8N1；实物驱动器必须设置一致 |
| BLD-305S 总线 | USART3 | PB10 TX / PB11 RX | 当前代码和 BLD-305S 手册均为 115200、8N1 |
...
当前工程选择开发板 ST-LINK V3 VCP，STM32 继续通过与 VCP 相连的 LPUART1 收发。Pi 已经通过该路径完成旧协议 ACK/STATUS 实测；本次不引入 MCU 原生 USB CDC。
...
当前工程型号已经是 STM32G474RE，无需再移植到 F446RE。2026-07-03 提交的待审固件已经完成：

- 保留已由 Pi 实测可通信的 ST-LINK V3 VCP + LPUART1 PC0/PC1。
- 使用 UART 中断和环形缓冲区接收，不在中断中解析 JSON。
- 解析 `v=1` 紧凑 W 帧及 CRC32。
- 返回带 CRC32 的 A/S 帧，并约 5 Hz 主动发送 STATUS。
- 实现 0.5 秒 watchdog、MKS/BLD Modbus 响应校验和 steer-before-drive。

当前旧版 `payload/type/sequence_id/wheels` JSON 已删除。详细改动见 `docs/STM32固件v1协议与电机驱动改动说明_2026-07-03.md`。

转向换算中的减速比固定为 `30.0`，并且只在 STM32 的 `mwrs_units.c` 中应用一次。真实输出仍默认锁定；STM32 负责人必须确认 SERVO57D 实物固件、机械零位、每轮方向符号、硬件急停和 homing 后才能解锁。
