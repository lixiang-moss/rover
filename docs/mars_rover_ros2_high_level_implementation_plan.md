...
这些事实会直接影响你的 ROS 2 方案：ROS 2 不应该绑定前一届的非 ROS 2 控制入口，而应该以标准 ROS 2 话题作为上层接口；同时 ROS 2 不应该假设底层驱动器型号已经完全确定，而应该通过 Pi 到 STM32 的抽象协议隐藏 BLD305S / BLD405S 差异。
...
| `/odom` | 无 | 无 | `nav_msgs/msg/Odometry` | 本项目手动控制不需要 |
