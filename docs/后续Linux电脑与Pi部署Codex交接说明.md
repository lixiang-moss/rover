...
### 4.5 Pi 与 STM32 已改用 USB 虚拟串口

旧检查中的 `/dev/serial0` 缺失和 `console=serial0` 占用属于已经放弃的 GPIO UART 方案，不再需要修复。任何后续 Codex 仍不能在没有 USB 枚举和协议测试证据时声称 Pi 到 STM32 已经可用。

在 STM32 联调前必须：

1. 用 USB 数据线连接 STM32G474RE 控制板。
2. 验证 `/dev/ttyACM*` 枚举和 `dialout` 权限。
3. 创建并验证 `/dev/mars-rover-stm32` udev 稳定别名。
4. 先运行 STM32 通信固件，只验证 CRC、ACK、STATUS 和 watchdog。
5. 再进入驱动器和电机测试。
...
- `/cmd_vel` 超时阈值约 0.5 秒。
- dry-run 不访问串口。
- 所有真实硬件 launch 默认 `STOP + disarmed`。
...
- Pi 能枚举 STM32 USB 虚拟串口。
- `/dev/mars-rover-stm32` 稳定别名可用。
- Pi 和 STM32 使用 USB 数据线连接。
- 若使用 ST-LINK VCP，其对应 UART 使用 115200、8N1；若使用原生 USB CDC，则不依赖 UART 帧格式。
...
- 不要在 dry-run 测试中打开串口。
- 不要在启动脚本中自动调用 arm；必须由操作者现场确认后手动调用服务。
- 不要把目标回显称为真实反馈。
...
test -e /dev/mars-rover-stm32 && readlink -f /dev/mars-rover-stm32 || echo MISSING
```
